...
public class IpdFragment extends BaseFragment {
    ...
    private void initView() {
        finalSupplyCheck.setChecked(fragmentActivity.ipdBean.isFinalSupply);
        finalSupplyCheck.setOnCheckedChangeListener((buttonView, isChecked) -> {
            fragmentActivity.ipdBean.isFinalSupply = isChecked;
        });
//        finalSupplyTv.setText(String.valueOf(fragmentActivity.entity.finalSupply));
        totalVolTv.setText(String.valueOf(fragmentActivity.ipdBean.peritonealDialysisFluidTotal));
        cycleVolTv.setText(String.valueOf(fragmentActivity.ipdBean.perCyclePerfusionVolume));
        cycleNumTv.setText(String.valueOf(fragmentActivity.ipdBean.cycle));
//        firstPerVolTv.setText(String.valueOf(fragmentActivity.entity.firstPerfusionVolume));
        ultVolTv.setText(String.valueOf(fragmentActivity.ipdBean.ultrafiltrationVolume));
        retainTimeTv.setText(String.valueOf(fragmentActivity.ipdBean.abdomenRetainingTime));
        finalRetainVolTv.setText(String.valueOf(fragmentActivity.ipdBean.abdomenRetainingVolumeFinally));
        lastRetainVolTv.setText(String.valueOf(fragmentActivity.ipdBean.abdomenRetainingVolumeLastTime));
                ...
                if (!TextUtils.isEmpty(result)) {
                    switch (mType) {
                        case PdGoConstConfig.CHECK_TYPE_PRESCRIPTION_PERITONEAL_DIALYSIS_FLUID_TOTAL: //腹透液总量
                            ...
                            fragmentActivity.ipdBean.peritonealDialysisFluidTotal = Integer.parseInt(result);
                            setCycleNum();
                            break;
                        case PdGoConstConfig.CHECK_TYPE_PRESCRIPTION_PER_CYCLE_PERFUSION_VOLUME: //每周期灌注量
                            cycleVolTv.setText(result);
                            fragmentActivity.ipdBean.perCyclePerfusionVolume = Integer.parseInt(result);
                            setCycleNum();
                            break;
...
//                            break;
                        case PdGoConstConfig.CHECK_TYPE_PRESCRIPTION_ESTIMATED_ULTRAFILTRATION_VOLUME: //预估超滤量
                            ...
                            fragmentActivity.ipdBean.ultrafiltrationVolume = Integer.parseInt(result);
                            setCycleNum();
                            break;
                    }
                }
    ...
    private void setCyclePre() {
        int perCyclePerfusionVolume = (fragmentActivity.ipdBean.peritonealDialysisFluidTotal
                - fragmentActivity.ipdBean.firstPerfusionVolume ////不需要扣除上次最末留腹量 - mActivity.fragmentActivity.entity.abdomenRetainingVolumeLastTime
                - fragmentActivity.ipdBean.abdomenRetainingVolumeFinally - 500 ) / fragmentActivity.ipdBean.cycle;
        cycleVolTv.setText(String.valueOf(perCyclePerfusionVolume));
        fragmentActivity.ipdBean.perCyclePerfusionVolume = perCyclePerfusionVolume;
//        CacheUtils.getInstance().getACache().put(PdGoConstConfig.TREATMENT_PARAMETER, mParameterEniity);
//        setTreatTime();
        int time = (fragmentActivity.ipdBean.abdomenRetainingTime * 60 * fragmentActivity.ipdBean.cycle) + ((fragmentActivity.ipdBean.firstPerfusionVolume + fragmentActivity.ipdBean.abdomenRetainingVolumeFinally +
                fragmentActivity.ipdBean.abdomenRetainingVolumeLastTime ) / 125);

        fragmentActivity.ipdBean.treatTime = EmtTimeUil.getTime(time);
    }
    ...
    private void setCycleNum() {
        ...
        int cycle =  (fragmentActivity.ipdBean.peritonealDialysisFluidTotal
                - fragmentActivity.ipdBean.firstPerfusionVolume ////不需要扣除上次最末留腹量 - mActivity.fragmentActivity.entity.abdomenRetainingVolumeLastTime
                - fragmentActivity.ipdBean.abdomenRetainingVolumeFinally - 500 ) / fragmentActivity.ipdBean.perCyclePerfusionVolume;
        if(cycle <= 0){
            cycle = 1;
        }
        fragmentActivity.ipdBean.cycle = cycle;
//        CacheUtils.getInstance().getACache().put(PdGoConstConfig.TREATMENT_PARAMETER, mParameterEniity);
