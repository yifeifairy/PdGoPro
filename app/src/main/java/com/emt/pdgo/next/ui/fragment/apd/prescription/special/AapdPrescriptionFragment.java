...
public class AapdPrescriptionFragment extends BaseFragment {
    ...
    @BindView(R.id.increaseTimeRl)
    RelativeLayout increaseTimeRl;
    @BindView(R.id.increaseTime)
    TextView increaseTime;

    @BindView(R.id.btnOk)
    Button btnOk;

    private TreatmentParameterEniity entity;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_aapd_prescription, container, false);
...
//        entity.peritonealDialysisFluidTotal = 1200;
        ...
        firstPerVolTv.setText(String.valueOf(entity.increaseVol));
        increaseTime.setText(String.valueOf(entity.increaseTime));
        ...
        firstPerVolRl.setOnClickListener(v -> {
            alertNumberBoardDialog(firstPerVolTv.getText().toString(), PdGoConstConfig.INCREASING_VOL);
        ...
        increaseTimeRl.setOnClickListener(v -> {
            alertNumberBoardDialog(increaseTime.getText().toString(), PdGoConstConfig.INCREASING_TIME);
        ...
        btnOk.setOnClickListener(v -> {
            MyApplication.isKid = false;
            if (checkConnectNetwork(Objects.requireNonNull(activity))) {
                addUserSnMachineRcp();
            }
            if (entity.firstPerfusionVolume == 0) {
                ...
                if (entity.firstPerfusionVolume >= entity.perCyclePerfusionVolume) {
                    ...
                    activity.toastMessage("每周期灌注量不能大于首次灌注量");
                }
            }
...
//        compositeDisposable.add(mainDisposable);
    ...
    private void alertNumberBoardDialog(String value, String type) {
        NumberBoardDialog dialog = new NumberBoardDialog(activity, value, type, false, true);
        dialog.show();
...
//                            setTotal();
                        ...
                        case PdGoConstConfig.CHECK_TYPE_PRESCRIPTION_ABDOMEN_RETAINING_VOLUME_LAST_TIME: //上次留腹量
                            ...
                            setCycleNum();
                            break;
                        case PdGoConstConfig.INCREASING_TIME: //上次留腹量
                            increaseTime.setText(result);
                            entity.increaseTime = Integer.parseInt(result);
                            break;
                        case PdGoConstConfig.INCREASING_VOL: //首次灌注量
                            firstPerVolTv.setText(result);
                            entity.increaseVol = Integer.parseInt(result);
//                            setTotal();
                            setCycleNum();
                            break;
